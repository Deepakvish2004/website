...
export default function AdminReports() {
  ...
  const uniqueUsers = Array.from(
    new Set(bookings.map((b) => b.user?.email || b.email))
  );
  const totalUsers = uniqueUsers.length;

  const userNames = Array.from(
    new Set(bookings.map((b) => b.user?.name || b.name || "N/A"))
  );

  // ✅ Helper: safe date parsing (use bookingDate consistently)
            ...
            <tbody>
              {bookingsForSelectedDate.map((b) => (
                ...
                >
                  ...
                  <td className="p-3">{b.user?.email || b.email || "N/A"}</td>
      ...
      <div className="mt-10 bg-green-600 p-6 rounded-2xl shadow-xl bg-gradient-to-r from-green-400 to-blue-700 ">
        <div className="bg-white shadow rounded-2xl p-6 mb-8">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-300 text-left m-5 rounded-lg p-5">
